...
// api:https://github.com/restorando/angular-pickadate
...
.directive('pickadate', ['$compile', '$document', '$window', 'pickadateUtils',
    'dateFilter',
    function ($compile, $document, $window, dateUtils, dateFilter) {

      var TEMPLATE =
        '<div class="pickadate" ng-show="displayPicker" ng-style="styles">' +
        '<div class="pickadate-header">' +
        '<div class="pickadate-controls">' +
        '<a href="" class="pickadate-prev" ng-click="changeMonth(-1)" ng-show="allowPrevMonth">prev' +
        '</a>' +
      ...
      return {
        ...
        link : function (scope, element, attrs, ngModel) {
          ...
          scope.setDate = function (dateObj) {
            if (isOutOfRange(dateObj.dateObj) || isDateDisabled(dateObj.date))
              return;
          ...
          if (wantsModal) {
            ...
            element.on('keydown', function (e) {
              if (indexOf.call([9, 13, 27], e.keyCode) >= 0)
                togglePicker(false);
            });
            ...
            $document.on('click', function (e) {
              ...
              if (isDescendant(compiledHtml[0], e.target) || e.target === element[0])
                return;
              togglePicker(false);
            });
          ...
          function render() {
            ...
            for (var i = 0; i < allDates.length; i++) {
              ...
              if (isDisabled)
                classNames.push('pickadate-unavailable');
              if (date === today)
                classNames.push('pickadate-today');
